...
try {
    ...
    $stmt = $pdo->query($query);
    $result = $stmt->fetch(PDO::FETCH_ASSOC);
    $stats['active_incidents'] = $result['count'] ?? 0;
    ...
    $stmt = $pdo->query($query);
    $result = $stmt->fetch(PDO::FETCH_ASSOC);
    $stats['total_volunteers'] = $result['count'] ?? 0;
    ...
    $completedIncidents = $incidentStats['completed'] ?? 0;
    $performance_percentage = $totalIncidents > 0 ? 
        round(($completedIncidents / $totalIncidents) * 100) : 0;
    ...
    // System alerts (certificates expiring soon)
    $query = "SELECT COUNT(*) as count FROM training_certificates 
              WHERE expiry_date BETWEEN CURDATE() AND DATE_ADD(CURDATE(), INTERVAL 30 DAY)";
    $stmt = $pdo->query($query);
    $result = $stmt->fetch(PDO::FETCH_ASSOC);
    $expiringCertificates = $result['count'] ?? 0;
    ...
    $equipmentStats['total'] = $equipmentStats['total'] ?? 0;
    $equipmentStats['operational'] = $equipmentStats['operational'] ?? 0;
    $equipmentStats['maintenance'] = $equipmentStats['maintenance'] ?? 0;
    $equipmentStats['offline'] = $equipmentStats['offline'] ?? 0;
...
<body>
                ...
                <div class="header-content">
                    ...
                    <div class="header-actions">
                        ...
                        <div class="user-profile">
                            <img src="../img/rei.jfif" alt="User" class="user-avatar" onerror="this.src='../img/default-avatar.png'">
                            <div class="user-info">
                                ...
                                <?php if (!empty($recentLogins)): ?>
                                    ...
                                    <div class="personnel-item">
                                        ...
                                        <div class="personnel-info">
                                            <p class="personnel-name"><?php echo htmlspecialchars($login['email'] ?? 'Unknown'); ?></p>
                                            <p class="personnel-details">Logged in at <?php echo date('H:i', strtotime($login['attempt_time'] ?? 'now')); ?></p>
                                        </div>
                                    </div>
                                    <?php endforeach; ?>
                                <?php else: ?>
                                    <div class="personnel-item">
                                        ...
                                        <div class="personnel-info">
                                            <p class="personnel-name">No recent activity</p>
                                            <p class="personnel-details">Check back later</p>
                                        </div>
                                    </div>
                                <?php endif; ?>
                            </div>
    ...
    <script>
            ...
            fetch('get_user_activity.php')
                ...
                .then(data => {
                    if (data.success && data.html) {
                        ...
                        const mockHtml = `
                            <div class="personnel-item">
                                <div class="personnel-icon icon-cyan"><i class='bx bxs-user icon-cyan'></i></div>
                                <div class="personnel-info">
                                    <p class="personnel-name">admin@example.com</p>
                                    <p class="personnel-details">Logged in at ${new Date().toLocaleTimeString()}</p>
                                </div>
                            </div>
                        `;
                        activityList.innerHTML = mockHtml;
                    }
        ...
        function toggleSubmenu(id) {
            ...
            const arrow = menuItem ? menuItem.querySelector('.dropdown-arrow') : null;

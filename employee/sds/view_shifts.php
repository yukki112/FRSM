...
// Check if user has permission (EMPLOYEE or ADMIN or USER who is a volunteer)
...
if (!$volunteer && $role !== 'EMPLOYEE' && $role !== 'ADMIN') {
...
// Get all shifts assigned to the user with duty assignments
function getUserShifts($pdo, $user_id, $volunteer_id, $role) {
    $sql = "SELECT 
                s.id,
                s.user_id,
                s.volunteer_id,
            ...
            LEFT JOIN attendance_logs a ON s.id = a.shift_id AND a.user_id = s.user_id
            LEFT JOIN users creator ON s.created_by = creator.id
            LEFT JOIN duty_assignments da ON s.duty_assignment_id = da.id
            WHERE ";
    
    // If user is a volunteer, show their volunteer shifts
    if ($volunteer_id) {
        $sql .= "s.volunteer_id = ?";
        $params = [$volunteer_id];
    } else {
        // If user is employee/admin, show their user shifts
        $sql .= "s.user_id = ?";
        $params = [$user_id];
    }
...
// Get all shifts (for admins/employees managing shifts) with duty assignments
function getAllShifts($pdo, $filter_status = 'all', $filter_date = '', $search_query = '') {
    $sql = "SELECT 
                s.id,
                s.user_id,
                s.volunteer_id,
...
function getShiftStats($pdo, $user_id, $volunteer_id) {
    $stats = [
        'total' => 0,
        'upcoming' => 0,
        'completed' => 0,
    ...
    // Get stats for user (either as user or volunteer)
    if ($volunteer_id) {
        // Volunteer stats
        $sql = "SELECT 
                    COUNT(*) as total,
                    ...
                    SUM(CASE WHEN confirmation_status = 'pending' THEN 1 ELSE 0 END) as pending_confirmation,
...
$view_type = isset($_GET['view']) ? $_GET['view'] : ($role === 'ADMIN' || $role === 'EMPLOYEE' ? 'all' : 'my');

// Get duty types for filter
$duty_types = getDutyTypes($pdo);

// Get shifts based on user role and view type
if ($role === 'ADMIN' || $role === 'EMPLOYEE') {
    if ($view_type === 'my') {
        $shifts = getUserShifts($pdo, $user_id, $volunteer_id, $role);
        $user_shifts = $shifts;
    } else {
        $shifts = getAllShifts($pdo, $filter_status, $filter_date, $search_query);
        $user_shifts = getUserShifts($pdo, $user_id, $volunteer_id, $role);
    }
} else {
    $shifts = getUserShifts($pdo, $user_id, $volunteer_id, $role);
    $user_shifts = $shifts;
...
// Apply duty type filter if selected
if ($filter_duty_type) {
    $shifts = array_filter($shifts, function($shift) use ($filter_duty_type) {
        return strcasecmp($shift['duty_type'] ?? '', $filter_duty_type) === 0;
    });
}

// Get shift statistics
$stats = getShiftStats($pdo, $user_id, $volunteer_id);
...
// View type options for admins/employees
$view_options = [
    'my' => 'My Shifts',
    'all' => 'All Shifts'
];
...
// Get duty type badge HTML
function getDutyTypeBadge($duty_type, $priority) {
    if (!$duty_type) return '<span class="no-duty">No Duty Assigned</span>';
    
    $priority_color = $GLOBALS['priority_colors'][$priority] ?? '#6b7280';
    $priority_text = ucfirst($priority);
    
    return <<<HTML
        <div class="duty-type-badge" style="border-left-color: {$priority_color}">
            <div class="duty-type-header">
                <span class="duty-type-title">{$duty_type}</span>
                <span class="duty-priority" style="background: {$priority_color}">{$priority_text}</span>
            </div>
        </div>
    HTML;
}
...
function getConfirmationBadge($status) {
    ...
    $status_colors = $GLOBALS['confirmation_colors'];
    $color = $status_colors[$status] ?? '#6b7280';
    $text = ucfirst(str_replace('_', ' ', $status));
    
    return <<<HTML
        <span class="confirmation-badge" style="background: rgba({hexToRgb($color)}, 0.1); color: {$color};">
            {$text}
        </span>
    HTML;
}
...
<head>
    ...
    <title>View Shifts - Fire & Rescue Management</title>
    ...
    <style>
        ...
        .duty-type-badge:hover::before {
            opacity: 1;
        }

        .duty-type-badge:hover {
...
<body>
    ...
    <div class="modal-overlay" id="log-modal">
        <div class="modal">
            <div class="modal-header">
                <h2 class="modal-title">Log Attendance</h2>
                <button class="modal-close" id="log-modal-close">&times;</button>
            </div>
            <div class="modal-body">
                <form id="log-attendance-form">
                    <input type="hidden" id="log-shift-id" name="shift_id">
                    
                    <div class="form-group">
                        <label class="form-label" for="check_in">Check-in Time</label>
                        <input type="datetime-local" class="form-input" id="check_in" name="check_in" required>
                    </div>
                    
                    <div class="form-group">
                        <label class="form-label" for="check_out">Check-out Time</label>
                        <input type="datetime-local" class="form-input" id="check_out" name="check_out">
                    </div>
                    
                    <div class="form-group">
                        <label class="form-label" for="attendance_notes">Notes (Optional)</label>
                        <textarea class="form-textarea" id="attendance_notes" name="attendance_notes" placeholder="Enter any notes about your attendance..."></textarea>
                    </div>
            ...
            <div class="menu-section">
                ...
                <div class="menu-items">
                    ...
                    <div id="schedule" class="submenu active">
                        <a href="create_shifts.php" class="submenu-item">Create Shift</a>
            ...
            <div class="dashboard-content">
                <div class="dashboard-header">
                    <div>
                        ...
                        <p class="dashboard-subtitle">Manage your shift assignments, confirm availability, and log attendance.
                        <?php if ($role === 'ADMIN' || $role === 'EMPLOYEE'): ?>
                            <?php echo $view_type === 'all' ? 'Viewing all shifts (Admin Mode)' : 'Viewing your assigned shifts'; ?>
                        <?php else: ?>
                            <?php echo $volunteer_id ? 'Viewing your volunteer shifts' : 'Viewing your assigned shifts'; ?>
                        <?php endif; ?>
                        </p>
                    </div>
                </div>
                ...
                <div class="content-container">
                    ...
                    <div class="quick-actions">
                        <a href="create_shifts.php" class="quick-action-card">
                            <div class="action-icon">
                                <i class='bx bxs-calendar-plus'></i>
                            </div>
                            <div class="action-content">
                                <div class="action-title">Create New Shift</div>
                                <div class="action-description">Schedule new shifts for volunteers or employees</div>
                            </div>
                        </a>
                        <a href="confirm_availability.php" class="quick-action-card">
                            <div class="action-icon">
                                <i class='bx bxs-check-shield'></i>
                            </div>
                            <div class="action-content">
                                <div class="action-title">Confirm Availability</div>
                                <div class="action-description">Review and confirm shift availability requests</div>
                            </div>
                        </a>
                        <a href="request_change.php" class="quick-action-card">
                            <div class="action-icon">
                                <i class='bx bxs-calendar-exclamation'></i>
                            </div>
                            <div class="action-content">
                                <div class="action-title">Shift Change Requests</div>
                                <div class="action-description">Manage shift change and swap requests</div>
                            </div>
                        </a>
                        <a href="../vra/view_availability.php" class="quick-action-card">
                            <div class="action-icon">
                                <i class='bx bxs-user-check'></i>
                            </div>
                            <div class="action-content">
                                <div class="action-title">View Volunteer Availability</div>
                                <div class="action-description">Check volunteer schedules and availability</div>
                            </div>
                        </a>
                    </div>
                    <?php endif; ?>
                    ...
                    <div class="stats-grid">
                        ...
                        <div class="stat-card" data-type="confirmed" onclick="showConfirmedShifts()">
                            <div class="stat-header">
                                <div class="stat-icon-container">
                                    <i class='bx bxs-check-shield'></i>
                                </div>
                    ...
                    <div class="filter-tabs-container">
                        <div class="filter-header">
                            ...
                            <div class="view-toggle">
                                <button class="view-btn <?php echo $view_type === 'my' ? 'active' : ''; ?>" onclick="changeView('my')">
                                    <i class='bx bxs-user'></i>
                                    My Shifts
                                </button>
                                <button class="view-btn <?php echo $view_type === 'all' ? 'active' : ''; ?>" onclick="changeView('all')">
                        ...
                        <div class="filter-tabs">
                            <a href="?status=all&date=<?php echo $filter_date; ?>&search=<?php echo urlencode($search_query); ?>&duty_type=<?php echo $filter_duty_type; ?>&view=<?php echo $view_type; ?>" class="filter-tab <?php echo $filter_status === 'all' ? 'active' : ''; ?>">
                                <i class='bx bxs-dashboard'></i>
                                All Shifts
                                <span class="filter-tab-count"><?php echo $stats['total']; ?></span>
                            </a>
                            <a href="?status=scheduled&date=<?php echo $filter_date; ?>&search=<?php echo urlencode($search_query); ?>&duty_type=<?php echo $filter_duty_type; ?>&view=<?php echo $view_type; ?>" class="filter-tab <?php echo $filter_status === 'scheduled' ? 'active' : ''; ?>">
                                <i class='bx bxs-time'></i>
                                Scheduled
                            </a>
                            <a href="?status=confirmed&date=<?php echo $filter_date; ?>&search=<?php echo urlencode($search_query); ?>&duty_type=<?php echo $filter_duty_type; ?>&view=<?php echo $view_type; ?>" class="filter-tab <?php echo $filter_status === 'confirmed' ? 'active' : ''; ?>">
                                <i class='bx bxs-check-circle'></i>
                                Confirmed
                            </a>
                            <a href="?status=completed&date=<?php echo $filter_date; ?>&search=<?php echo urlencode($search_query); ?>&duty_type=<?php echo $filter_duty_type; ?>&view=<?php echo $view_type; ?>" class="filter-tab <?php echo $filter_status === 'completed' ? 'active' : ''; ?>">
                                <i class='bx bxs-check-double'></i>
                                Completed
                            </a>
                            <a href="?status=cancelled&date=<?php echo $filter_date; ?>&search=<?php echo urlencode($search_query); ?>&duty_type=<?php echo $filter_duty_type; ?>&view=<?php echo $view_type; ?>" class="filter-tab <?php echo $filter_status === 'cancelled' ? 'active' : ''; ?>">
                                <i class='bx bxs-x-circle'></i>
                                Cancelled
                            </a>
                            <?php if ($volunteer_id): ?>
                            <a href="?status=scheduled&date=&search=&duty_type=&view=<?php echo $view_type; ?>" class="filter-tab <?php echo empty($filter_status) || $filter_status === 'pending' ? 'active' : ''; ?>">
                                <i class='bx bxs-hourglass'></i>
                                Pending Confirmation
                                <span class="filter-tab-count"><?php echo $stats['pending_confirmation']; ?></span>
                            </a>
                            <?php endif; ?>
                        </div>
                    </div>
                            ...
                            <form method="GET" id="filter-form">
                                <div class="filter-row">
                                    ...
                                    <div class="filter-group">
                                        ...
                                        <input type="text" class="filter-input" name="search" placeholder="Search by unit, location, duty type..." value="<?php echo htmlspecialchars($search_query); ?>">
                                    </div>
                                </div>
                        ...
                        <div class="shifts-table-container" style="max-height: 500px;">
                            <?php if (count($shifts) > 0): ?>
                                <?php foreach ($shifts as $index => $shift): ?>
                                    ...
                                    // Determine who this shift is assigned to
                                    if ($shift['shift_for'] === 'volunteer') {
                                    ...
                                    if (isset($shift['assigned_to_name']) && $shift['assigned_to_name']) {
                                        $assignedTo .= ': ' . $shift['assigned_to_name'];
                                    }
                                    
                                    // Check if this shift belongs to current user
                                    $isMyShift = ($shift['user_id'] == $user_id) || ($shift['volunteer_id'] == $volunteer_id);
                                    ?>
                                    <div class="table-row" style="animation-delay: <?php echo $index * 0.05; ?>s;">
                                        ...
                                        <div class="table-cell" data-label="Duty Assignment">
                                            <?php echo getDutyTypeBadge($shift['duty_type'], $shift['priority']); ?>
                                            <?php if ($shift['duty_description']): ?>
                                                <div class="duty-description"><?php echo htmlspecialchars(substr($shift['duty_description'], 0, 50)); ?>...</div>
                                            <?php endif; ?>
                                        </div>
                                        ...
                                        <div class="table-cell" data-label="Actions">
                                            <div class="action-buttons">
                                                ...
                                                <?php if ($shift['status'] !== 'cancelled' && $shift['status'] !== 'completed'): ?>
                                                    <?php if ($isMyShift || $role === 'ADMIN' || $role === 'EMPLOYEE'): ?>
                                                        <button class="action-button log-button" onclick="logAttendance(<?php echo $shift['id']; ?>)" 
                                                            <?php echo $shift['check_in'] ? 'disabled' : ''; ?>>
                                                            <i class='bx bxs-log-in'></i>
                                                            <?php echo $shift['check_in'] ? 'Logged' : 'Log In'; ?>
                                                        </button>
                                                    <?php endif; ?>
                                                    
                                                    <?php if ($volunteer_id && $shift['confirmation_status'] === 'pending' && $isMyShift): ?>
                                                        <button class="action-button confirm-button" onclick="confirmShift(<?php echo $shift['id']; ?>)">
                                                            <i class='bx bxs-check-shield'></i>
                                                            Confirm
                                                        </button>
                                                    <?php endif; ?>
                            ...
                            <?php else: ?>
                                <div class="no-shifts">
                                    ...
                                    <?php if ($filter_status !== 'all' || $filter_date !== '' || $search_query !== '' || $filter_duty_type !== ''): ?>
    ...
    <script>
        ...
        function initEventListeners() {
            ...
            // Log attendance form submission
            const logForm = document.getElementById('log-attendance-form');
        ...
        function renderShiftDetails(shift) {
                ...
                <div class="shift-details">
                    <div style="display: flex; justify-content: space-between; align-items: flex-start; margin-bottom: 24px;">
                        ...
                        <div style="display: flex; gap: 8px;">
                            <span class="status-badge status-${shift.status.toLowerCase()}">
                                ${shift.status.toUpperCase()}
                            </span>
                            <span class="shift-type-badge shift-type-${shift.shift_type.toLowerCase()}">
                                <i class='bx ${shift.shift_type_icon}'></i>
                    ...
                    <div style="background: rgba(59, 130, 246, 0.05); border-radius: 12px; padding: 16px; margin-bottom: 20px; border-left: 4px solid var(--info);">
                        <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 8px;">
                            <div>
                                ...
                                <div style="font-size: 18px; font-weight: 600; color: var(--info);">${shift.unit_name}</div>
                            </div>
            ...
            detailsHtml += `
                    ...
                    <div style="background: var(--card-bg); border: 1px solid var(--border-color); border-radius: 12px; padding: 16px; margin-bottom: 20px;">
                        <div style="font-size: 12px; color: var(--text-light); margin-bottom: 4px;">Assigned To</div>
                        <div style="font-size: 16px; font-weight: 600;">${shift.assigned_to || 'N/A'}</div>
                    </div>`;
                    ...
                    <div style="background: linear-gradient(135deg, rgba(139, 92, 246, 0.05), rgba(139, 92, 246, 0.1)); border-radius: 12px; padding: 20px; margin-bottom: 20px; border-left: 4px solid var(--purple);">
                        ...
                        <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(250px, 1fr)); gap: 16px; margin-bottom: 16px;">
                            ...
                            <div style="background: var(--card-bg); border: 1px solid var(--border-color); border-radius: 8px; padding: 12px;">
                                ...
                                <div>
                                    <span class="duty-priority" style="background: ${shift.priority_color}">
                    ...
                    <div style="background: linear-gradient(135deg, rgba(16, 185, 129, 0.05), rgba(16, 185, 129, 0.1)); border-radius: 12px; padding: 20px; margin-bottom: 20px; border-left: 4px solid var(--success);">
                        ...
                        <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 16px;">
                            <div style="background: var(--card-bg); border: 1px solid var(--border-color); border-radius: 8px; padding: 12px;">
                                ...
                                <div style="font-size: 14px; font-weight: 600;">${new Date(shift.check_in).toLocaleString()}</div>
                            </div>`;
                ...
                if (shift.check_out) {
                    detailsHtml += `
                            <div style="background: var(--card-bg); border: 1px solid var(--border-color); border-radius: 8px; padding: 12px;">
                                ...
                                <div style="font-size: 14px; font-weight: 600;">${new Date(shift.check_out).toLocaleString()}</div>
                            </div>
                            <div style="background: var(--card-bg); border: 1px solid var(--border-color); border-radius: 8px; padding: 12px;">
                                <div style="font-size: 11px; color: var(--text-light); text-transform: uppercase; letter-spacing: 0.5px; margin-bottom: 4px;">Total Hours</div>
                                <div style="font-size: 14px; font-weight: 600;">${shift.total_hours} hours</div>
                            </div>`;
                }
        ...
        function logAttendance(shiftId) {
            const logModal = document.getElementById('log-modal');
            const logShiftId = document.getElementById('log-shift-id');
            ...
            checkInInput.value = formatForInput(now);
            checkOutInput.value = '';
            logShiftId.value = shiftId;
        ...
        function submitAttendance() {
            ...
            .then(data => {
                if (data.success) {
                    showNotification('success', data.message || 'Attendance logged successfully!');
                    closeLogModal();
                    setTimeout(() => {
                        location.reload(); // Reload to show updated attendance
                    }, 1500);
                } else {
                    showNotification('error', data.message || 'Failed to log attendance');
                }
            })
            .catch(error => {
        ...
        function confirmShift(shiftId) {
            if (confirm('Are you sure you want to confirm this shift?')) {
                ...
                .then(data => {
                    if (data.success) {
                        showNotification('success', data.message || 'Shift confirmed successfully!');
                        setTimeout(() => {
                            location.reload();
                        }, 1500);
                    } else {
                        showNotification('error', data.message || 'Failed to confirm shift');
                    }
                })
                .catch(error => {
                    showNotification('error', 'Error: ' + error.message);
